...
function FileUpload({ onChange, id, link, titleTooltip, accept, pattern, error, className }) {
  ...
  const handleChange = (e) => {
    ...
    if (file) {
      reader.onload = (e) => {
        ...
        if (onChange) {
          onChange(idInput, file, handleOnloading, handleSuccess, handleNormal, handleError);
        }
      }
  ...
  return (
    <div className={className}>
      ...
      <input type="file" id={idInput} accept={acceptInput} style={{ display: 'none' }} onChange={handleChange} ref={fileNode} />
