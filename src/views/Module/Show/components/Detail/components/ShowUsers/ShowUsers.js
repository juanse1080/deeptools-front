...
import { Avatar, Tooltip, IconButton, Typography, makeStyles, Icon, Paper, Fab, Grid } from '@material-ui/core'
      ...
      value.users.length > 0 ? value.users.map((user, index) =>
        <div key={user.id} className={classes.hoverUser}>
          <Tooltip title="Delete user" placement="bottom">
            <Paper className={classes.icon + " actions"} style={{ top: 'calc(25% - 15px)' }}>
              <Icon fontSize="small" className="fal fa-times-circle" onClick={onDeleting(index)} />
            </Paper>
          </Tooltip>
          <Tooltip title={title(`${user.first_name} ${user.last_name}`)} placement="bottom">
            <Avatar alt={user.first_name} src={`${host}${user.photo}`} className={classes.large + " avatar"} />
          </Tooltip>
          <Tooltip title="Show user" placement="bottom">
            <Paper className={classes.icon + " actions"} style={{ bottom: 'calc(25% - 15px)' }}>
              <Icon fontSize="small" className="fal fa-user-circle" onClick={show(user.id)} />
            </Paper>
          </Tooltip>
        </div>
    ...
    <Grid container justify="flex-end" className="mt-3">
      <Grid item>
        <Tooltip title="Add user">
          <Fab size="small" color="primary" aria-label="Add user" onClick={onCreating}>
