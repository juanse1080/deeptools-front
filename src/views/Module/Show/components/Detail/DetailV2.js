...
export default function Detail({ module, handle }) {
  ...
  const tryDeleting = index => () => {
    ...
    setDeleting({
      name: title(`${tempUser.first_name} ${tempUser.last_name}`),
      id: tempUser.id,
      index: index
    });
  ...
  const deleted = () => {
    axios
      ...
      .then(function(res) {
        console.log(module.users[deleting.index], [
          ...module['no_subscribers'],
          module.users[deleting.index]
        ]);
  ...
  const algorithms = () => {
    dispatch(actions.startLoading());
    history.push(
      user.role === 'developer' || module.subscribers.includes(user.id)
        ? '/module'
        : '/subscriptions'
    );
    dispatch(actions.finishLoading());
    ...
    axios
      ...
      .then(function(res) {
        ...
        if (res.data === 'add') {
          ...
          handle(
            'subscribers',
            module.subscribers.filter(item => item !== user.id)
          );
        }
      })
      .catch(function(err) {
  ...
  const steps =
    module.owner || module.role === 'admin'
      ? [
          {
            label: 'Description',
  ...
  return (
    <div>
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url(${host}${module.background})`
        }}>
        <Grid
          container
          justify="center"
          direction="row"
          style={{ position: 'absolute', top: 10, left: 10, width: 'auto' }}>
          <Grid item xs={12}>
            <Breadcrumbs
        ...
        <Tooltip
          ...
          title={
            module.subscribers.length !== 1
              ? `${module.subscribers.length} users`
              : 'One user'
          }>
          <Chip
            variant="outlined"
            icon={
              <Icon fontSize="small" className={clsx('fal fa-user-circle')} />
            }
            label={module.subscribers.length}
            size="small"
            classes={{ outlined: classes.outlinedChip }}
          />
        </Tooltip>
      </div>
      <Tabs
          ...
          {sm ? (
            ...
            <Grid item>
              {module.subscribers.includes(user.id) || module.owner ? (
                ...
                <Tooltip title="Subscribe">
                  <IconButton onClick={toggleSubscriber} className="mr-2">
                    <Icon fontSize="small" className="fal fa-anchor" />
                  </IconButton>
                </Tooltip>
              )}
      ...
      <Dialog
        ...
        <DialogActions>
          ...
          <Button onClick={deleted} variant="outlined">
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog
        ...
        <DialogContent>
          <Autocomplete
            ...
            renderInput={params => (
              <TextField {...params} label="Images" variant="outlined" />
            )}
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={cancelAdd} variant="outlined">
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
