...
import { authHeaderJSON, host, ws } from 'helpers';
...
export default function({ match, ...others }) {
  ...
  const addDescription = state => {
    ...
    setProgress([...data]);
    if (data[data.length - 1].state === 'success') setBuild(false);
  };
  ...
  useEffect(() => {
    axios
      ...
      .then(function(res) {
        ...
        const graphs = res.data.elements_type.filter(
  ...
  return (
    <>
      {!build && !loading ? (
        <Detail module={module} handle={handleUser} />
      ) : null}
