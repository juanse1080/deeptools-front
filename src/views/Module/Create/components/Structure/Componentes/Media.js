...
export default function ({ change, element }) {
  ...
  const { transform, opacity } = useSpring({
    ...
    transform: `perspective(600px) rotateX(${element.value === 'video' ? 180 : 0}deg)`,
  ...
  const toggleMedia = () => {
    change({ ...element, value: element.value === 'video' ? 'image' : 'video' })
  }
  ...
  return <>
    <div className={classes.root} onMouseEnter={() => set({ scale: 1.3 })} onMouseLeave={() => set({ scale: 1 })}>
      <AnimatedCard className={classes.card} style={{ opacity: opacity.interpolate(o => 1 - o), transform }}>
        <CardMedia
      ...
      <AnimatedCard className={classes.card} style={{ opacity, transform: transform.interpolate(t => `${t} rotateX(180deg)`) }} >
        <CardMedia
          className={classes.media}
          image={video}
          title="Contemplative Reptile"
      ...
      <FormHelperText className={classes.help}>
        Quantity: {element.len > 0 ? `Many ${element.value}s` : `One ${element.value}`}
      </FormHelperText>
      <AnimatedIconButton
