...
export default function({ value, change, errors, input, ...others }) {
  ...
  const handleType = () => {
    setExtensions(extensions =>
      extensions === 'default' ? 'custom' : 'default'
    );
  };
      ...
      <Grid container>
        <Grid className="p-2" item sm={12} xs={12}>
          <Typography
            ...
            component="p"
            className="mb-3">
            Help us with some necessary information to be able to communicate
            with your container.
          </Typography>
        </Grid>
        ...
        <Grid className="p-2" item sm={6} xs={12}>
          <TextField
            ...
            name="workdir"
            onChange={setPath}
            error={hasError('workdir')}
            helperText={
              hasError('workdir') ? errors.workdir[0] : 'Path to workdir'
            }
          />
        </Grid>
        <Grid className="p-2" item sm={6} xs={12}>
          <TextField
            ...
            error={hasError('file')}
            helperText={
              hasError('file') ? errors.file[0] : 'how is the file is named?'
            }
          />
        </Grid>
        <Grid className="p-2" item sm={6} xs={12}>
            ...
            {extensions === 'default' ? null : (
              <Grid item sm={extensions === 'default' ? 12 : 6} xs={12}>
                <TextField
                  ...
                  name="extensions"
                  onChange={setValue}
                  error={hasError('extensions')}
                  helperText={
                    hasError('extensions')
                      ? errors.extensions[0]
                      : 'Allowed extensions separated by spaces'
                  }
                />
              </Grid>
            )}
        ...
        <Grid className="p-2" item sm={6} xs={12}>
          <InputFile
            onChange={change}
            id="background"
            titleTooltip="Select an image that represents your algorithm"
            accept="image/*"
            error={hasError('background') ? errors.background[0] : null}
            name="background"
          />
        </Grid>
        <Grid className="p-2" item sm={12} xs={12}>
        ...
        <Grid className="p-2" item sm={12} xs={12}>
          <TextField
            ...
            helperText={hasError('description') ? errors.description[0] : null}
          />
        </Grid>
      </Grid>
    </>
